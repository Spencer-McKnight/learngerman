...
**2026-08-14 — Stage 6 "Szenen": the episode rework (lesson coherence, prefetching, generation reliability).** User feedback on the first-draft flows: steps felt disconnected, no storytelling, sessions short, speaking tasks showed no translation, and step 1 died on "No usable text". Rebuilt the session around ONE generated episode per session.

- **Episode architecture:** `engine/session/episode.ts` — one `generateObject` call produces a whole scene (story Act 1 → dialogue Act 2 → cloze/construct/shadow items lifted from or continuing it → conversation opener → recap), validated as a whole. Composer emits `plan.episodeSpec` + per-task `section` refs instead of per-task GenerationSpecs; `/api/generate` is gone, replaced by `/api/episode`. The client fetches it during the briefing (scene card with title/setting; Go disabled until resolved) — zero mid-session spinners; conversation opener ships inside the episode so the top rung starts instantly.
- **Reliability ladder (root cause of "No usable text"):** (1) `gatewayOptions` used undocumented fields (`sort`/`caching`) AND a plain model string — providerOptions.gateway only works with the `gateway()` wrapper, so fallbacks never engaged; fixed with documented fields (`models`/`user`/`tags`). (2) Models tiered in `lib/ai.ts`: `EPISODE_MODEL` gemini-2.5-flash, `CHAT_MODEL` flash-lite — both verified free-tier accessible live (gemini-3-flash, haiku-4.5, gpt-5-nano are NOT; free tier also rate-limits per-minute). (3) Validation is now two-floor: strict (drives one repair attempt with named offenders) then lenient acceptance (scene coverage floor −10%; missing targets and per-item construct/opener coverage forgiven — one stray word in a 7-word sentence is 12% coverage, and every word is tappable now). (4) `NoObjectGeneratedError` is re-rolled, not fatal; schema array maxima removed (models overshoot maxItems; client takes what it needs via `usableClozeItems`). (5) Terminal failures serve hand-written fallback episodes (`fallback-episodes.ts`, 3 scenes fully inside the seed lexicon, picked by stage) — the app now works with no AI key at all.
- **Task UI:** shared `GlossText` makes EVERY resolvable word tappable (episode glosses → lexicon lookup with gender article); `LineWithMeaning` puts each line's EN one tap away; shadowing/scripted show the meaning of lines the learner must speak (fix for "speaking a sentence without translating it"). Retrieval feedback shows the scene line carrying the word ("Aus der Szene: …"). Every task opens with a one-line why (`t.tasks.intro.*`). Done screen shows the episode's recap.
- Live-verified on a fresh session: real scene "Im Treppenhaus" generated (attempt 2, lenient), coherent across story → taps → dialogue → bite → shadowing; second session generated a continuity scene with the same neighbour. 71 vitest tests (episode contract + fallback shape covered), lint and build clean.
- Watch: episode generation is ~35s/attempt on flash (worst case ~72s absorbed by the briefing screen); each learner-state change means a fresh generation, cache only dedupes identical specs. Proper names (Lena, Ben) burn the 2-token out-of-lexicon slack — a capitalisation-aware name allowance would let more attempts pass strict.
